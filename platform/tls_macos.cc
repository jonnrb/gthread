...
tls::tls() {
  ...
  memcpy(after(), *g_pthread_self_slot, pthread_t_size);
  memset(after() + pthread_t_size, 0, k_num_slots * sizeof(void*));

  // set the user tcb to be this tls block
  void** tls_slots = (void**)(after() + pthread_t_size);
  tls_slots[0] = after();
}

tls::~tls() {
  void** tls_slots = (void**)(after() + get_pthread_slots_offset());
...
size_t tls::postfix_bytes() {
  return sizeof(tls) + k_num_slots * sizeof(void*) + get_pthread_slots_offset();
}
