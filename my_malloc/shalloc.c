...
// after the partition, and  the current Page_Internal's size is readjusted to
// reflect the size partitioned
void* allocatenew(Page_Internal* Page_Internalptr, int size) {
  Page_Internal* temptr = NULL;
  // Calculate the space left over after allocating enough for the current
  // request
  int totalspace = Page_Internalptr->space;
  totalspace = totalspace - (size + sizeof(Page_Internal));
  // temptr is a pointer that will increment to the location after the requested
  ...
  // the input to the function is then adjusted to indicate the correct size
  // requested  and its active 'used' flag
...
void* shalloc(size_t size) {
  ...
  else if (Page_Internalptr->space >= size + sizeof(Page_Internal) &&
           Page_Internalptr->used == FALSE) {
...
// returns a pointer to the Page_Internal that is before the Page_Internal being
// freed
...
// used for concatenation of two unused Page_Internals in the main free function
// returns a pointer to the Page_Internal that is after the Page_Internal being
// freed
...
void myfreeShalloc(void* p) {
  ...
  // At this point, the pointer exists, and the Page_Internal corresponding to
  // it is valid  The Page_Internal's flag is set to unused = 'FALSE'
  ptr->used = FALSE;
  ...
  // concatenates the space of the right Page_Internal, and the current
  // Page_Internal
  if (rightp != NULL && rightp->used == FALSE) {
    ptr->space = ptr->space + rightp->space + sizeof(Page_Internal);
    rightp = NULL;
  }
  // concatenates the space of the left Page_Internal, and the current
  // Page_Internal
