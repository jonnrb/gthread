...
extern int gthread_is_rb_correct(gthread_rb_tree_t tree);

typedef struct item {
  char label;

  gthread_rb_node_t node;
  uint64_t sort_key;
} item_t;
...
void iot_1(gthread_rb_tree_t tree, size_t* size) {
  ++*size;
  if (tree != NULL) {
...
void iot_2(gthread_rb_tree_t tree, item_repr_t* arr, size_t* i, size_t depth) {
  if (tree == NULL) {
...
void print_tree(gthread_rb_tree_t tree) {
  size_t size = 0;
  iot_1(tree, &size);
...
void assert_tree_invalid() {
  ...
  assert(gthread_is_rb_correct(NULL));
  ...
  assert(!gthread_is_rb_correct(&items[3].node));
}
...
void construct_and_insert(gthread_rb_tree_t* tree, item_t* item, char label,
                          uint64_t sort_key) {
  item->label = label;
  item->sort_key = sort_key;
  gthread_rb_construct(&item->node);

  gthread_rb_push(tree, &item->node);

  if (!gthread_is_rb_correct(*tree)) {
...
void stress() {
  ...
  for (size_t i = 0; i < num_nodes; ++i) {
    gthread_rb_node_t* popped = gthread_rb_pop_min(&tree);
    assert(popped != NULL);
    if (!gthread_is_rb_correct(tree)) {
...
void time_test() {
  ...
  for (size_t num_nodes = 8; num_nodes < max_num_nodes; num_nodes <<= 1) {
    ...
    int64_t start = gthread_clock_process();
    gthread_rb_tree_t tree = NULL;
    item_t items[num_nodes];
    for (size_t i = 0; i < num_nodes; ++i) {
      construct_and_insert(&tree, items + i, 'X', nums[i]);
    }
    int64_t t = gthread_clock_process() - start;
    printf("adding  %4zu things took %9" PRId64 " ns (log => %5.02f)\n",
